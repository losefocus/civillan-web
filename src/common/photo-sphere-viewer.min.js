var PhotoSphereViewer = function (args) {
  ...
  this.load = function () {
    container.innerHTML = "";
    if (!!loading_html && loading_html.nodeType === 1) container.appendChild(loading_html); else if (!!loading_html && typeof loading_html == "string") container.innerHTML = loading_html; else if (!!loading_img) {
    ...
    viewer_size = {width: 0, height: 0, ratio: 0};
  ...
  var autorotate = function () {
    ...
    triggerAction("position-updated", {longitude: long, latitude: lat});
    render();
  ...
  var fitToContainer = function () {
    if (container.clientWidth != viewer_size.width || container.clientHeight != viewer_size.height) {
      resize({width: container.clientWidth, height: container.clientHeight})
    }
  };
  ...
  this.getPositionInDegrees = function () {
    return {longitude: long * 180 / Math.PI, latitude: lat * 180 / Math.PI}
  };
  ...
  var retrieveKey = function (evt) {
    ...
    if (evt.keyCode || evt.which) {
      ...
      var keycodes_map = {38: "ArrowUp", 39: "ArrowRight", 40: "ArrowDown", 37: "ArrowLeft"};
  ...
  this.toggleFullscreen = function () {
    if (!isFullscreenEnabled()) enableFullscreen(); else disableFullscreen()
  };
  ...
  var setNewViewerSize = function (size) {
    for (var dim in size) {
      if (dim == "width" || dim == "height") {
        ...
        new_viewer_size[dim] = {css: size_value + size_unit, unit: size_unit}
      }
    }
  };
  ...
  var captured_view = {horizontal_fov: 360, vertical_fov: 180};
  ...
  if (args.overlay !== undefined) {
    if (args.overlay.image !== undefined) {
      overlay = {image: args.overlay.image, position: {x: "left", y: "bottom"}};
...
var Sphoords = function () {
  ...
  this.getCoordinatesInDegrees = function () {
    return {longitude: long_deg, latitude: lat_deg}
  };
  ...
  var executeListeners = function () {
    if (!!listeners.length) {
      for (var i = 0, l = listeners.length; i < l; ++i) {
        listeners[i]({longitude: long, latitude: lat})
      }
    }
  };
  var recording = false;
...
Sphoords.getScreenOrientation = function () {
  ...
  if (!!screen.orientation) screen_orientation = screen.orientation; else if (!!screen.mozOrientation) screen_orientation = screen.mozOrientation; else if (!!screen.msOrientation) screen_orientation = screen.msOrientation; else if (!!window.orientation || window.orientation === 0) switch (window.orientation) {
    case 0:
